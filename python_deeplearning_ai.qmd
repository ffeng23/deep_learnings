...
+ Machine learning specicialization

https://learn.deeplearning.ai/specializations/machine-learning/lesson/dhmb8/optional-lab:-gradient-descent-for-logistic-regression
...
# Gradient descent with multiple linear regression and logistic regression

## multiple linear regression

Do multiple linear regression first!


$$

w_j=w_j- \alpha \frac{d J(w,b)}{d w_j}  
...
$$
  ...
  \frac{d J(w,b)}{d w_j} = \frac{1}{m} \sum_{i=1}^{m}(f_{w,b}(X^{(i)}) - y^{(i)})x_j

$$

$$

  \frac{d J(w,b)}{d b} = \frac{1}{m} \sum_{i=1}^{m}(f_{w,b}(X^{(i)}) - y^{(i)})

$$

## logistic regression

Now it is the case for logistic regression.

To make it straight. Gradient descent for Logistic regression is the same as that for the linear regrssion, except that the function is sigmoid (look at the last equation down below).
...
**Note:**, $f_{w,b}(X^{(i)})$ is the sigmoid function below,

$$

f_{w,b}(X^{(i)})=\frac{1}{1+e^{-(\mathbf{w}\cdot\mathbf{x}^{(i)}+b)}}
